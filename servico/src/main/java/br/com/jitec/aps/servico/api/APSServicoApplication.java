...
@OpenAPIDefinition(
	tags = { 
			@Tag(name = ServicoApiConstants.TAG_ORDENS_SERVICO, description =  ServicoApiConstants.TAG_ORDENS_SERVICO_DESCRIPTION),
    ...
    security = {
            @SecurityRequirement(name = "servicoSecurityScheme")
    },
    components = @Components(
            securitySchemes = {
            		@SecurityScheme(securitySchemeName = "servicoSecurityScheme", type = SecuritySchemeType.HTTP, scheme = "Bearer", bearerFormat = "JWT")
            }
    )
)
